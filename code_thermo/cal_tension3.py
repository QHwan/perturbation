...
denMat = np.loadtxt(denFileName)
denMat = np.transpose(denMat)
dVec = denMat[1]

pk2Vec = np.zeros(len(pkVec))
kBT = 2.479*280/298.
pref = 1.6603	# kJ/mol/nm3 -> MPa
for i in range (len(rVec)):
	pk2Vec[i] = dVec[i]*kBT*pref
...
pkEVec = np.zeros(len(rVec))
pkSVec = np.zeros(len(rVec))

for i in range (len(rVec)):
	pkSVec[i] = (pk2Vec[i]-pkVec[i])/20.*300

for i in range (len(rVec)):
	pkEVec[i] = pkVec[i] + pkSVec[i]
...
for i in range (len(rVec)):
	ptEVec[i] = -dEVec[i] + 3*pkEVec[i] - pnEVec[i]
	ptEVec[i] *= 0.5

for i in range (len(rVec)):
	ptSVec[i] = -dSVec[i] + 3*pkSVec[i] - pnSVec[i]
	ptSVec[i] *= 0.5
