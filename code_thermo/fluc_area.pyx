...
def main():
	...
	cdef float v
	cdef float pref
	cdef float eps, epsp, epsm, dAp, dAm, dV
		...
		if mVec[frame] != nH2O and nFrame != 1:
			...
			for i in range (nH2O):
				...
				for j in range (i+1, nH2O):
					...
					coord_pert_area(iMat, bufMat, epsp)
					coord_pert_area(jMat, bufMat, epsp)
			...
			tension1 = dUp1_avg/Ap
			tension2 = -0.5/kBT*((dUp2_avg-dUp1_avg*dUp1_avg)/Ap) 
			tension3 = 1./6/kBT/kBT*((dUp3_avg - 3*dUp2_avg*dUp1_avg + 2*dUp1_avg*dUp1_avg*dUp1_avg)/Ap)

			tension1 *= pref
			tension2 *= pref
			tension3 *= pref

			#oMat.append([tVec[frame-1],buf_dU1/(Ap), -0.5/kBT*(buf_dU2-buf_dU1*buf_dU1)/Ap, 1./6/kBT/kBT*(buf_dU3 - 3*buf_dU2*buf_dU1 + 2*buf_dU1*buf_dU1*buf_dU1)])
